...
#from initweight import Weight

from net_factory import GCN7
...
#model.load_state_dict(torch.load(os.path.join(predir, 'final.pt')))
...
def val(epoch):
    ...
    with torch.no_grad():
        for data, target, idx in val_loader:
            ...
            prec1, = accuracy(output, target) # test precison in one batch
            acc.update(prec1.item(), target.size(0))
    print('\nVal set: Average loss: {:.4f}, Accuracy: {:.2f}%'.format(test_loss.avg, acc.avg))
    return acc.avg
